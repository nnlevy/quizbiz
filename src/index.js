...
function renderProgram(){if(!q('#programReady'))return;const p=programState(),r=readiness(p),n=r.filter(x=>x[1]).length,programCanSubmit=n>=5&&Boolean((p.eventName||'').trim());q('#programReady').textContent=n;if(q('#programReadyHero'))q('#programReadyHero').textContent=n+'/6';q('#programChecklist').innerHTML=r.map(x=>'<li>'+(x[1]?'Ready':'Needs detail')+': '+x[0]+'</li>').join('');q('#programSample').textContent='Quizbiz LLC: Reminder for '+(p.eventName||'your program')+'. Reply YES to confirm, STOP to unsubscribe, or HELP for help. Msg frequency varies. Msg & data rates may apply.';if(q('#saveProgram'))q('#saveProgram').disabled=!programCanSubmit;if(q('#programReadyState')){const needed=5-Math.min(n,5);q('#programReadyState').className='readiness '+(programCanSubmit?'is-ready':'is-warn');q('#programReadyState').textContent=programCanSubmit?'Ready to save approval evidence.':'Not ready yet. '+needed+' more check'+(needed===1?'':'s')+' needed.';}}
