...
const STEP_TEXT=[['Define the program','Set the audience, message purpose, sender identity, and public opt-in path.'],['Load the cohort','Import contacts with role, segment, phone number, consent source, and suppression status.'],['Sync engagement','Reconcile Microsoft Forms, Outlook responses, Zoom attendance, and manual check-ins.'],['Automate follow-up','Send reminders, confirmations, thank-yous, missed-you notes, and reporting summaries.']];
let activeStep=0;
...
function trackCursor(){if(!window.matchMedia||window.matchMedia('(pointer: coarse)').matches||window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;document.addEventListener('pointermove',(event)=>{document.body.style.setProperty('--mx',event.clientX+'px');document.body.style.setProperty('--my',event.clientY+'px')},{passive:true})}
